...
// Start a new search
router.post('/', (0, validation_1.validateRequest)(validation_1.searchValidationSchema), async (req, res, next) => {
    try {
        const { businessType, location, maxResults = 20 } = req.body;
        // Create search job in database
        const searchJob = new SearchJob_1.SearchJob({
            businessType,
            location,
            status: 'PENDING'
        });
        ...
        await (0, queue_1.addScrapingJob)(jobData);
        logger_1.logger.info('Search job created', {
...
router.get('/:searchId', async (req, res, next) => {
    try {
        ...
        if (!searchId.match(/^[0-9a-fA-F]{24}$/)) {
            throw new errorHandler_1.CustomError('Invalid search ID format', 400);
        }
        const searchJob = await SearchJob_1.SearchJob.findById(searchId);
        if (!searchJob) {
            throw new errorHandler_1.CustomError('Search not found', 404);
        }
        // Get businesses found so far
        const businesses = await Business_1.Business.find({ searchId })
            .sort({ extractedAt: -1 })
            .limit(100);
        const response = {
            success: true,
...
router.get('/', async (req, res, next) => {
    try {
        ...
        const searches = await SearchJob_1.SearchJob.find()
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit);
        const total = await SearchJob_1.SearchJob.countDocuments();
        const response = {
            success: true,
            data: {
                searches,
...
router.get('/:searchId/export', async (req, res, next) => {
    try {
        ...
        if (format === 'csv') {
            ...
            const csvRows = businesses.map(business => `"${business.name}","${business.address}","${business.phone || ''}","${business.website || ''}","${business.emails.join('; ')}","${business.source}","${business.extractedAt}"`).join('\n');
...
router.delete('/:searchId', async (req, res, next) => {
    try {
        ...
        await Business_1.Business.deleteMany({ searchId });
        // Delete the search job
        await SearchJob_1.SearchJob.findByIdAndDelete(searchId);
        logger_1.logger.info('Search deleted', { searchId });
        const response = {
            success: true,
